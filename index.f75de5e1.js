...
function getUniqueCities() {
    const tableBodyRows = [
        ...document.querySelector("tbody").rows
    ];
    return [
        ...new Set(tableBodyRows.map((row)=>row.cells[2].textContent))
    ];
}
function getForm() {
    ...
    tableHead.forEach((el)=>{
        ...
        if (title === "Office") {
            ...
            const typeInput = title === "Age" || title === "Salary" ? "number" : "text";
...
function createNewEmployee() {
    const employee = {
        ...
        city: [
            ...document.getElementsByName("office")
        ][0].value,
        age: +[
            ...document.getElementsByName("age")
        ][0].value,
        salary: +[
            ...document.getElementsByName("salary")
        ][0].value
    };
...
function getEmployee(employee) {
    ...
    Object.keys(employee).forEach((key)=>{
        ...
        cell.textContent = key === "salary" ? `$${employee[key].toLocaleString("en")}` : employee[key];
        row.append(cell);
    });
    return row;
}
